...
Picker startup pre-creates only the official Picker session. An earlier experiment also preloaded the Google Photos page in a minimized popup, but that introduced an unreliable hidden-window lifecycle and could recreate a page the user had closed. Production no longer opens any Google Photos page before a user click. The click path atomically consumes the standby session and creates one visible popup directly from that click; a replacement session begins prewarming while the user is selecting. This trades Google Photos document-load time for a predictable, user-controlled window lifecycle.
